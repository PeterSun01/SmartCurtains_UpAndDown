...
static void Motor_Height_100to90(void)
{
    ...
    while(Motor_Status==MOTORSTATUS_RUN);
    Motor_Stop();       
...
#define ANGLE_TIME        350

void Motor_Angle_Measure_UP(void)
{
    Motor_Status=MOTORSTATUS_RUN;
    Motor_Up();
    esp_timer_start_once(timer_once_handle, ANGLE_TIME * 1000); //启动单次定时器
    while(Motor_Status==MOTORSTATUS_RUN);
    Motor_Stop();       
...
void Motor_Angle_Measure_DOWN(void)
{
    Motor_Status=MOTORSTATUS_RUN;
    Motor_Down();
    esp_timer_start_once(timer_once_handle, ANGLE_TIME * 1000); //启动单次定时器
...
int Motor_AutoCtl(int8_t auto_height,int8_t auto_angle)//自动控制函数，给高角值
{
    ...
    else if((auto_height==100)&&(Motor_Value.height==0))//目标为100，x  当前是0，0，则先执行全放
    {
        Motor_SetAllDown();
